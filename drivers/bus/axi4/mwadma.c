...
void mwadma_tx_cb_continuous_signal_dataflow(struct mwadma_dev *mwdev)
{
    ...
    {
        if(current_transfers_queued >= 1)
        {
            /* If more than one transfer is pending it is safe enough to premeptively start a new transfer */
            mwadma_start(mwdev,mwchan);

            if(current_transfers_queued >= tx_wm.full)
            {
                mwchan->error = TX_ERROR_QFULL;
                kill_fasync(&mwdev->asyncq, SIGIO, POLL_OUT);
                sysfs_notify(&dev->kobj, NULL, "dma_ch2");
            }
            else if(current_transfers_queued > (tx_wm.prime))
            {
        ...
        /* There are no new pending transfers */
        else if(current_transfers_queued == 0) 
        {
            spin_lock_bh(&mwchan->slock);
            mwchan->status = finishing;
            spin_unlock_bh(&mwchan->slock);
            ...
            sysfs_notify(&dev->kobj, NULL, "dma_ch2");
            dev_dbg(dev, "No queued Tx\n");
        }
    }
    else if(mwchan->status == ready)
    {
        return;
    }
    else if(current_transfers_queued > 1)
    {
        mwadma_start(mwdev,mwchan);
        ...
        dev_dbg(dev, "Fast resume\n");

    }
    /* Still not enough data to resume */
    else
    {
        spin_lock_bh(&mwchan->slock);
        mwchan->status = waiting;
        spin_unlock_bh(&mwchan->slock);
        dev_dbg(dev, "No fast resume\n");
    }
}
    ...
    switch(cmd)
    {
        ...
        case MWADMA_TX_ENQUEUE:
            ...
            if( (status == ready) && (transfer_queued == 0))
            {
                spin_lock_bh(&mwchan->slock);
                new = list_entry(mwchan->curr->list.next,struct mwadma_slist,list);
                mwchan->next_index =  new->buffer_index;
                spin_unlock_bh(&mwchan->slock);
            }
            else
            {
            ...
            dev_dbg(&IP2DEV(mwdev), "Synced ring index = %d\n", mwchan->next_index);
#endif
            ...
            if(unlikely((status == waiting) && (transfer_queued > tx_wm.low))) /* restart if required */
            {
        ...
        case MWADMA_TX_CONTINUOUS:
            ...
            transfer_queued = mwchan->transfer_queued;
            spin_unlock_bh(&mwchan->slock);
            if(transfer_queued > tx_wm.low) /* restart ifrequired */ {
        ...
        struct mw_axidma_params *usrbuf)
{
    ...
    dev_dbg(&IP2DEV(mwdev), "Bytes per ring         :%d\n", mwchan->ring_bytes);
